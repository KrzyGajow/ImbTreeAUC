BuildTreeInter <- function( node, Y_name, X_names, data, depth, min_obs, type, levelPositive, cp, n_cores, weights, AUCweight, cost, 
  ...
  if( nrow(split_rule) !=0 ){
    ...
    split_rule <- InteractiveLearning( split_rule, node, Y_name, X_names, data, depth, min_obs, type, levelPositive, cp, n_cores, 
  ...
  if( all(probability %in% c(0,1)) | {depth <= 0} | {nrow(split_rule) == 0} | {node$Count / 2 <= min_obs} ){
    ...
    BuildTreeInter( childl, Y_name, X_names, child_frame[[2]], depth - 1, min_obs, type, levelPositive, cp, n_cores, 
                    weights, AUCweight, cost, class_th, overfit, cf, amb_prob, top_split, var_lev, amb_class, amb_class_freq, 2 * number, tree_path )
    
    # Recursive call of the building function (BuildTreeInter) for the right child
    BuildTreeInter( childr, Y_name, X_names, child_frame[[1]], depth - 1, min_obs, type, levelPositive, cp, n_cores, 
                    weights, AUCweight, cost, class_th, overfit, cf, amb_prob, top_split, var_lev, amb_class, amb_class_freq, 2 * number + 1, tree_path )
