...
SplitLocalNum <- function( variable, target, measure_parent, indexes, k_unique, min_obs, type, levelPositive, cp, 
  ...
  if( n_cores == 1 ){
    ...
    for( i in seq_along(k_unique) ){
      ...
      n_obs_child <- sum( variable <= k_unique[i] )
      ...
      if( any( n_obs_child < min_obs ) ){
        ...
        if( type == "AUCl"){
          ...
          temp <- SplitAucLocal( variable, target, k_unique[i], measure_parent, levelPositive, cp, weights, AUCweight, cost, overfit )

        }else if(type == "AUCg"){
          
          temp <- SplitAUC( variable, target, k_unique[i], measure_parent, indexes, levelPositive, cp, weights, AUCweight, cost, overfit )
    ...
    clusterExport(Global_Cluster, c("AUCweight", "CalcProb", "weights", "cost", "cp", "variable",  "target", 
                                    "measure_parent", "indexes", "levelPositive", "type", "k_unique", "n_obs",
                                    "ClassErrorLocal", "AvoidSameClass", "overfit", "Probability_matrix"), 
                  envir = environment())
...
SplitLocalFac <- function( variable, target, measure_parent, indexes, k_unique, min_obs, type, levelPositive, cp, n_cores, 
                           weights, AUCweight, cost, overfit ){
  ...
  if( n_cores == 1 ){
    ...
    for( i in seq_along(k_unique) ){
      ...
      # Split variable into two parts, i.e. less or equal to the split point and greater then the split point
      # Calculate number of observations in each child
      n_obs_child <- sum( variable %in% k_unique[1:i] )
      n_obs_child <- c( n_obs_child, n_obs - n_obs_child)
      ...
      if( any( n_obs_child < min_obs ) ){
        ...
        if( type == "AUCl"){
          ...
          temp <- SplitAucLocal( variable, target, k_unique[1:i], measure_parent, levelPositive, cp, weights, AUCweight, cost, overfit )
          
        }else if(type == "AUCg"){
          
          temp <- SplitAUC( variable, target, k_unique[1:i], measure_parent, indexes, levelPositive, cp, weights, AUCweight, cost, overfit )
