ImbTreeAUCInterShiny1 <- function( Y_name, X_names, data, depth = 5, min_obs = 5, type = "AUCg", levelPositive = "1", cp = 0, n_cores = 1,
                                   weights = NULL, AUCweight = "none", cost = NULL, class_th = "equal", overfit = "leafcut", cf = 0.25, 
                                   amb_prob =  1, top_split = 2, var_lev = T, amb_class = NULL, amb_class_freq = NULL, shiny = list() ){
  ...
  type_ <- type
  type <- ifelse( type == "AUCs", "AUCg", type )
  ...
  if( !type_ == "AUCg" ){
    ...
    BuildTreeInterShiny( Tree, Y_name, X_names, data, depth, min_obs, type, levelPositive, cp, n_cores, weights, AUCweight, 
                         cost, class_th, overfit, cf, amb_prob, top_split, var_lev, amb_class, amb_class_freq, 1, shiny )
...
ImbTreeAUCInterShiny2 <- function(){
  ...
  AddAttr( imb$n0$Tree, imb$n0$data, imb$n0$Y_levels, imb$n0$Y_statistics, imb$n0$min_obs, imb$n0$levelPositive, 
           imb$n0$type, imb$n0$cp, imb$n0$weights, imb$n0$AUCweight, imb$n0$cost, imb$n0$class_th, NULL, imb$n0$overfit,
...
BuildTreeInterShiny <- function( node, Y_name, X_names, data, depth, min_obs, type, levelPositive, cp, n_cores, weights, AUCweight, cost, 
  ...
  if( process$wait == T ){
    ...
    probability <- CalcProb( data, Y_name, if( is.null(weights) ){ NULL }else{ weights[node$indexes] }, cost )
    ...
    split_rule <- BestSplitGlobalInter( data[, c(X_names, Y_name)], Y_name, node$measure, min_obs, type, levelPositive, cp, 
                                        node$indexes, n_cores, weights, AUCweight, cost, overfit )
    ...
    if( nrow(split_rule) !=0 ){
      ...
      split_rule <- InteractiveLearningShiny( split_rule, node, Y_name, X_names, data, depth, min_obs, type, levelPositive, cp, n_cores, weights, 
                                              AUCweight, cost, class_th, overfit, cf, amb_prob, top_split, var_lev, class_prob_learn, number, shiny )
  ...
  if( Skip( lname ) ){
    ...
    BuildTreeInterShiny( get(env_name, envir = imb)$childl, get(env_name, envir = imb)$Y_name, 
                         ...
                         get(env_name, envir = imb)$type, get(env_name, envir = imb)$levelPositive, 
                         get(env_name, envir = imb)$cp, get(env_name, envir = imb)$n_cores, 
                         get(env_name, envir = imb)$weights, get(env_name, envir = imb)$AUCweight, 
                         get(env_name, envir = imb)$cost, 
...
InteractiveLearningShiny <- function( split_rule, tree, Y_name, X_names, data, depth, min_obs, type, levelPositive, cp, n_cores, weights, 
                                      AUCweight, cost, class_th, overfit, cf, amb_prob, top_split, var_lev, amb_class, number, shiny ){
...
CrossValid <- function( Y_name, X_names, dat, type, depth, min_obs, levelPositive, weights, cost, AUCweight, overfit, cp, cf, k_fold, seed ){
  
  if( c("prune") %in% overfit ){
    
    Grid1 <- expand.grid( Type = type, Depth = depth, Min_obs = min_obs, Overfit = overfit[which("prune"!=overfit)], 
                          Cp = cp, Cf = 0.25, stringsAsFactors = F )
    Grid2 <- expand.grid( Type = type, Depth = depth, Min_obs = min_obs, Overfit = "prune", Cp = 0, Cf = cf, 
                          stringsAsFactors = F )
    Grid <- rbind( Grid1, Grid2 )
    
  }else{
    
    Grid <- expand.grid( Type = type, Depth = depth, Min_obs = min_obs, Overfit = overfit, Cp = cp, Cf = 0.25, 
                         stringsAsFactors = F )
    ...
    withProgress(message = "Tuning process", detail = sprintf("1 out of %s", nrow(train)), value = 0, {
      ...
      for( i in 1:length(cross_validation) ){
        ...
        for( j in 1:nrow(Grid) ){
          ...
          Tree <- ImbTreeAUC( Y_name = Y_name, X_names = X_names, data = dat[ cross_validation[[i]] == 1, ], type = Grid[j,"Type"], 
                              ...
                              levelPositive = levelPositive, AUCweight = AUCweight )
  ...
  train_agg <- aggregate( train[,c("Accuracy","Kappa","Nleaves","Nclass")], as.list(train[,2:7]), mean )
  valid_agg <- aggregate( valid[,c("Accuracy","Kappa","Nleaves","Nclass")], as.list(valid[,2:7]), mean )
