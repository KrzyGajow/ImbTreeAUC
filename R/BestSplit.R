BestSplitLocal <- function( variable, target, measure_parent, min_obs, type, levelPositive, cp, indexes, n_cores, weights, AUCweight, cost, overfit ){
  ...
  if( is.factor(variable) ){
    ...
    if( !is.ordered(variable) ){
      ...
      if( length( levels(target) ) == 2 ){
        ...
        levels_ordPCA <- PCAorder( target, variable, weights, cost )
...
BestSplitGlobal <- function( data, Y_name, measure_parent, min_obs, type, levelPositive, cp, indexes, n_cores, weights, AUCweight, cost, overfit ) {
  ...
  best_splits_local <- sapply( colnames(data)[X_variables], function(x){
      ...
      BestSplitLocal( data[,x], data[,Y_name], measure_parent, min_obs, type, levelPositive, cp, indexes, n_cores, weights, AUCweight, cost, overfit )
