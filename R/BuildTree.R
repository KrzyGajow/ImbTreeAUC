BuildTree <- function( node, Y_name, X_names, data, depth, min_obs, type, levelPositive, cp, n_cores, 
                       weights, AUCweight, cost, class_th, overfit, number ){
  ...
  split_rule <- BestSplitGlobal( data[, c(X_names, Y_name)], Y_name, node$measure, min_obs, type, levelPositive, cp, node$indexes, n_cores,
                                 weights, AUCweight, cost, overfit )

  # Check if the improvement is greater than the threshold
  no_improvement <- IfImprovement( node, split_rule, cp )
  ...
  if( all(probability %in% c(0,1)) | {depth <= 0} | {no_improvement} | {node$Count / 2 <= min_obs} ){
    ...
    BuildTree( childl, Y_name, X_names, child_frame[[2]], depth - 1, min_obs, type, levelPositive, cp, n_cores, 
               weights, AUCweight, cost, class_th, overfit, 2 * number )
    ...
    BuildTree( childr, Y_name, X_names, child_frame[[1]], depth - 1, min_obs, type, levelPositive, cp, n_cores, 
               weights, AUCweight, cost, class_th, overfit, 2 * number + 1 )
...
IfImprovement <- function( node, split_rule, cp ){
